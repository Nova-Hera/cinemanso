    ...
    <div class="flex flex-col gap-8">
        ...
        <div style="display:flex; gap:2.5rem; align-items:flex-start;">
            ...
            <div style="flex-shrink:0; width:8rem;">
                <div style="width:8rem; height:12rem; overflow:hidden; border:1px solid rgb(255 255 255)"
                    ...
                    <div class="w-full h-full bg-zinc-800 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-zinc-600" fill="none"
                ...
                <dl class="mt-4 space-y-2 text-sm">
                    ...
                    <dd class="text-zinc-800 dark:text-zinc-200">
                        {{ $movie->release_date ? $movie->release_date->format('d/m/Y') : '—' }}</dd>
            </div>
            <div>
                <dt class="text-xs font-semibold tracking-wide text-zinc-500 dark:text-zinc-400">ASSISTIDO EM
                </dt>
                <dd class="text-zinc-800 dark:text-zinc-200">
                    {{ $movie->watched_at ? $movie->watched_at->format('d/m/Y') : '—' }}</dd>
            </div>
            ...
            <div class="mt-6 border-t border-zinc-200 dark:border-zinc-700 pt-4">
                <dt class="text-xs font-semibold tracking-wide text-zinc-500 dark:text-zinc-400 mb-2">ONDE ASSISTIR
                    (BR)</dt>
                <div class="flex gap-2 flex-wrap">
                    @foreach($movie->streamings['BR'] as $streaming)
                    <img src="{{ $streaming['logo'] }}" alt="{{ $streaming['name'] }}" title="{{ $streaming['name'] }}"
            ...
            <div
                ...
                <!-- Ajustado para text-sm e adicionado leading-tight para ficar alinhado na sidebar -->
                <h3
                    ...
                    <span>Reproduzir no Stremio</span>
                </h3>

                <div class="flex flex-col gap-2.5">
                    <!-- 1. Botão que ABRE O APP (padding reduzido para px-3) -->
                    ...
                    <!-- 2. Botão STREMIO WEB (texto levemente encurtado e padding reduzido) -->
                ...
                <!-- Texto de aviso reduzido para caber sem poluir -->
                <p class="mt-3 text-[11px] leading-tight text-zinc-500 dark:text-zinc-400">
        ...
        <div class="flex-1 min-w-0 flex flex-col gap-6">
            ...
            @php
            $medalColor = fn ($r) => $r == 1 ? 'color:rgb(190,160,0)' : ($r == 2 ? 'color:rgb(192,192,192)' : ($r ==
            3 ? 'color:rgb(184,115,51)' : null));
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-4 flex-wrap">
                    <h1 class="text-3xl font-black leading-tight tracking-tight" style="color:rgb(23,221,98);">
                        {{ $movie->title }}</h1>
                    <span
                        style="{{ $statusBadge['style'] }}; border-radius:9999px; padding:0.2rem 0.6rem; font-size:0.75rem; font-weight:600; flex-shrink:0;">
                        {{ $statusBadge['label'] }}
                    </span>
                ...
                <div class="text-sm text-zinc-500 dark:text-zinc-400 flex flex-wrap gap-x-2 gap-y-0.5">
                    ...
                    <span>Em {{ $g }}:
                        ...
                        <span class="font-bold"
                            style="{{ $medalColor($rank) }}">#{{ $rank }}/{{ $genreTotals[$g] }}</span>
                        @else
            ...
            @if ($media !== null)
            <div style="display:flex; justify-content:space-between; padding:1rem 0; border-bottom:1px solid rgb(228 228 231);"
                class="dark:border-zinc-700">
                <div class="text-center">
                    <div class="text-4xl font-bold tabular-nums" style="color:rgb(23,221,98)">
                        {{ rtrim(rtrim(number_format($mediana, 2, '.', ''), '0'), '.') }}
                    </div>
                    <div class="text-xs font-semibold tracking-wide text-zinc-500 dark:text-zinc-400 mt-1">MEDIANA
                    </div>
                </div>
                <div class="text-center">
                    <div class="text-4xl font-bold tabular-nums" style="color:rgb(23,221,98)">
                    ...
                    <div class="text-xs font-semibold tracking-wide text-zinc-500 dark:text-zinc-400 mt-1">MÉDIA
                    </div>
                </div>
                <div class="text-center">
                    <div class="text-4xl font-bold tabular-nums" style="color:rgb(23,221,98)">
                    ...
                    <div class="text-xs font-semibold tracking-wide text-zinc-500 dark:text-zinc-400 mt-1">MODA
                    </div>
                </div>
            </div>
            @else
            ...
            <div style="display:flex; align-items:center; gap:1rem; flex-wrap:wrap;">
                ...
                <div x-data="{ showCreateForm: false, showEditForm: false }">
                    ...
                    @if ($userReview === null)
                    <flux:button @click="showCreateForm = true" style="background:rgb(0,123,24);color:#fff;border:none"
                    ...
                    <div class="fixed inset-0 bg-black/50 z-40" x-cloak x-show="showCreateForm || showEditForm"
                    ...
                    <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50
                        ...
                        <form method="POST" action="{{ route('reviews.store') }}"
                            ...
                            <flux:button type="submit" variant="primary" class="w-full"
                                style="background:rgb(0,123,24);color:#fff;border:none">Publicar</flux:button>
                        </form>
                    </div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('reviews.store') }}"
                        ...
                        <flux:input type="number" name="rating" max="10" min="0" step="0.1" label="Nota"
                            placeholder="8.5" required />
                        <x-review-editor name="content" placeholder="O que você achou?" label="Review"
                            :required="true" />
                        <flux:button type="submit" variant="primary" class="w-full"
                            style="background:rgb(0,123,24);color:#fff;border:none">Publicar</flux:button>
                    </form>
                ...
                <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50
                    ...
                    <form method="POST" action="{{ route('reviews.update', $userReview->id) }}"
                        ...
                        <flux:textarea class="hover:bg-sky-700" name="content" label="Review" required>
                            {{ $userReview->content }}</flux:textarea>
                        <flux:button type="submit" variant="primary" class="w-full"
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700
            ...
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    <img src="{{ asset('storage/' . ($review->user->profile_picture ?: 'default-profile.png')) }}"
                        alt="{{ $review->user->name }}" class="h-7 w-7 rounded-full object-cover flex-shrink-0" />
                    <span class="font-semibold text-sm"><a
                            href="{{route('users.show',['user'=>$review->user->username])}}">{{ $review->user->name }}</a></span>
                </div>
