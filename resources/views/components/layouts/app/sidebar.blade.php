...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    ...
    *:has(> [data-flux-main]) {
    ...
    <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navbar class="me-1.5 space-x-0.5 rtl:space-x-reverse py-0!">
            <flux:tooltip :content="__('Search')" position="bottom">
                <flux:navbar.item as="button" class="!h-10 [&>div>svg]:size-5" icon="magnifying-glass"
                    :label="__('Search')" onclick="Livewire.dispatch('open-search')" />
            </flux:tooltip>
            @auth
            <flux:tooltip content="Novo Filme" position="bottom">
                <flux:navbar.item as="button" class="!h-10 [&>div>svg]:size-5" icon="plus-circle"
                    onclick="Livewire.dispatch('open-new-movie')" />
            </flux:tooltip>
            @endauth
        </flux:navbar>
        ...
        <flux:dropdown position="top" align="end">
            <button type="button" class="cursor-pointer rounded-lg p-1 hover:bg-zinc-200/60 dark:hover:bg-zinc-800">
                <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                    ...
                    @else
                    <span
                        class="flex h-full w-full items-center justify-center rounded-lg bg-neutral-200 text-black dark:bg-neutral-700 dark:text-white text-xs font-semibold">C</span>
                    @endauth
                </span>
            </button>
            <flux:menu>
                    ...
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                                ...
                                @else
                                <span
                                    class="flex h-full w-full items-center justify-center rounded-lg bg-neutral-200 text-black dark:bg-neutral-700 dark:text-white">C</span>
                                @endauth
                            </span>
                            <div class="grid flex-1 text-start text-sm leading-tight">
                                <span
                                    class="truncate font-semibold">{{ auth()->check() ? auth()->user()->name : __('Convidado') }}</span>
                                <span
                                    class="truncate text-xs">{{ auth()->check() ? auth()->user()->email : __('') }}</span>
                            </div>
                        </div>
                    </div>
                </flux:menu.radio.group>
                ...
                @auth
                <flux:menu.radio.group>
                    <flux:menu.item :href="route('settings.profile')" icon="cog" wire:navigate>{{ __('Configurações') }}
                    </flux:menu.item>
                </flux:menu.radio.group>
                <flux:menu.separator />
    ...
    {{-- Left sidebar --}}
    <flux:sidebar sticky class="app-sidebar border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        <div class="sidebar-pad flex items-center gap-2 px-2 py-2 sidebar-center">
            <button id="sidebarToggle" type="button"
                ...
                <svg class="toggle-icon h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
        ...
        <div class="sidebar-scroll-area sidebar-pad px-2">
            <flux:navlist variant="outline">
                <flux:navlist.group class="grid">
                    ...
                    <flux:navlist.item icon="film" :href="route('movies.index')"
                        :current="request()->routeIs('movies.index')" wire:navigate class="sidebar-nav-item">
                        <span class="sidebar-fade">Filmes</span>
                    </flux:navlist.item>

                    @if(auth()->check())
                    <flux:navlist.item icon="arrow-path" :href="route('wheel')" :current="request()->routeIs('wheel')"
                        wire:navigate class="sidebar-nav-item">
                        <span class="sidebar-fade">Roleta</span>
                    </flux:navlist.item>

                    {{-- Insira junto com os outros itens de navlist.group --}}
                    <flux:navlist.item icon="tv" :href="route('neko')" :current="request()->routeIs('neko')"
                        wire:navigate class="sidebar-nav-item">
                        <span class="sidebar-fade">Neko</span>
                    </flux:navlist.item>
                    @endif

                    <div class="recent-divider">
                        <span
                            class="sidebar-fade text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                            Recém-acessados
                        </span>
